...
class DP832:
    def __init__(self, usb_or_serial='USB0'):
        try:
            ...
            #
            # self.address = [elem for elem in self.instrument_list if (elem.find('tty') != -1 and elem.find(
            #     usb_or_serial) != -1)]  # Search a instrument with USB and serial number in the instrument list

            self.address = [elem for elem in self.instrument_list if elem.find('tty') != -1 ]
            print(self.address)
    ...
    def toggle_output(self, chan, state):
        outstr = "ON" if state else "OFF"
        self.device.write(":OUTP CH" + str(chan) + "," + outstr)
